...
datosCoordinador = open(r'C:\Users\NalV\Documents\Definitive_CE_DATA\October\28\c0_3tx_1220.log','r')
datosTransmisor1 = open(r'C:\Users\NalV\Documents\Definitive_CE_DATA\October\28\com4_3tx_1220' , 'r')
datosTransmisor2 = open(r'C:\Users\NalV\Documents\Definitive_CE_DATA\October\28\com7_3tx_1220' , 'r')
datosTransmisor3 = open(r'C:\Users\NalV\Documents\Definitive_CE_DATA\October\28\com15_3tx_1220' , 'r')
        ...
        if dat != "\n":
            for l in dat:
                if l == "Success\n":
                    ...
                    time_suc_t1.append(timee.replace("]",""))
                if l == "01\n":
                    c_error1_t1  += 1
...
print(c_error2_t1 , c_error2_t2 , c_error3_t3)
...
print("El valor promedio de RSSI para cada transmisor es de :  " ,av1 , av2, av3)
print("Varianza RSSI:", vari_addr1, vari_addr2, vari_addr3)
print("Desviacion Estandar RSSI", desv1 , desv2 , desv3)
...
if ( len(rssi_addr1) and len(rssi_addr2) )>0:
    ...
    fig0 = plt.figure()
    plt.axhline(y=av1,color='blue',linestyle='-', lw = 2)
    plt.axhline(y=av2,color='green',linestyle='-', lw = 2)
    plt.axhline(y=av3,color='cyan',linestyle='-', lw = 2)
    ...
    plt.plot(x11 , dbm_addr1 , ls = ':' , lw = 0.1 , c = 'blue' , marker= '+' , ms = 10 , mec='blue' , label = 'E1')
    plt.plot(x22 , dbm_addr2 , ls = ':' , lw = 0.1 , c = 'green' , marker= '2' , ms = 10 , mec='green', label ='E2')
    if len(rssi_addr3)>0:     
        x33 = np.arange(0,len(dbm_addr3))
        plt.plot(x33 , dbm_addr3 , ls = ':' , lw = 0.1 , c = 'cyan' , marker= '3' , ms = 10 , mec='cyan', label ='E3')
...
while f < (len(time_suc_t2)):  
           ...
           f +=1
k=0
while k < (len(time_suc_t3)):  
           if k==0 or k%2 ==0:
               y3.append(time_suc_t3[k])
           else:
               x3.append(time_suc_t3[k])
           k +=1           
...
datenums3 = md.date2num(timest3)


datenums4 = md.date2num(datex1)
datenums5 = md.date2num(datex2)
datenums6 = md.date2num(datex3)
...
xfmt = md.DateFormatter('%H:%M:%S')
ax.xaxis.set_major_formatter(xfmt)
fig4.suptitle('RSSI values referred to the timestamp for E1 (RxSide)')
plt.plot(datenums4,dbm_addr1)
...
xfmt = md.DateFormatter('%H:%M:%S')
ax.xaxis.set_major_formatter(xfmt)
fig5.suptitle('RSSI values referred to the timestamp for E2 (RxSide)')
plt.plot(datenums5,dbm_addr2)
...
if len(rssi_addr3)>0:
    ...
    xfmt = md.DateFormatter('%H:%M:%S')
    ax.xaxis.set_major_formatter(xfmt)
    fig6.suptitle('RSSI values referred to the timestamp for E3 (RxSide)')
    plt.plot(datenums6,dbm_addr3)
